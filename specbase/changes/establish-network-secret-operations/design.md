...
- `tests/tooling/environment.bats` executes the flake-provided tool catalogue on every supported system and fails when `sops`, `age`, `ssh-to-age`, `tofu`, or retained commands resolve outside `repo-tools` or are absent. It proves reproducible command availability, not remote authentication.
...
1. Extend the pinned operator tool catalogue, `tooling.md`, the native harness runtime inputs/default Bats list, and existing environment test.
