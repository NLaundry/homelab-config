...
- [ ] 1.1 Add locked `sops-nix` input wiring; add `sops`, `age`, `ssh-to-age`, Neovim, and OpenSSH's `ssh-keygen`/`ssh-keyscan` to the selected cross-platform operator tool set.
...
- [ ] 2.1 Manually run `command -v sops age age-keygen ssh-to-age nvim ssh-keygen ssh-keyscan` in each supported Nix development shell before following the secret runbook; stop if a required command is missing.
