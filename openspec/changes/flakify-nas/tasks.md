...
- [ ] 4.3 `make build` — confirms it builds on the NAS (remote build works from macOS) — **needs the NAS; run on the box**

## 5. Deploy + verify on the box

- [ ] 5.1 `make test` — non-persistent trial activation
- [ ] 5.2 Verify: `ssh operator@10.10.10.11` succeeds with key (no password), `sudo whoami` → root with no prompt
- [ ] 5.3 Verify: `zpool status` shows `smolBoy` + `mediaBin` ONLINE and imported (no `-f` needed on boot)
- [ ] 5.4 `make deploy` — persist as the boot generation
- [ ] 5.5 Reboot check (optional): pools import automatically, box reachable

## 6. Housekeeping

- [x] 6.1 Update `README.md` with deploy usage (`make deploy`, host layout)
- [ ] 6.2 Commit `flake.lock` and note update cadence — cadence documented in README; **commit left to you** (staged, not committed)
